...
// GET /api/user-contacts/[authorId]/notes - Get notes for a user's author contact
  ...
  try {
    ...
    const [notes, total] = await Promise.all([
      prisma.contactNote.findMany({
        where: { userContactId: userContact.id },
      ...
      prisma.contactNote.count({ where: { userContactId: userContact.id } }),
    ]);
    ...
    console.error('Error fetching user contact notes:', error);
    return NextResponse.json(
...
// POST /api/user-contacts/[authorId]/notes - Create a note for a user's author contact
export async function POST(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('Error creating user contact note:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
// PATCH /api/user-contacts/[authorId]/notes - Update a note
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('Error updating user contact note:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
...
// DELETE /api/user-contacts/[authorId]/notes - Delete a note
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    console.error('Error deleting user contact note:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
